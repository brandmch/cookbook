...
export default function RecipeCard({ cookbookSlug, recipe }: RecipeCardProps) {
  ...
  return (
    ...
    >
      {/* Image area */}
      {recipe.emoji ? (
        <div className="flex items-center justify-center bg-amber-50 rounded-t-[3px] h-36 text-6xl relative">
          <span>{recipe.emoji}</span>
          <span className={`cat-stamp ${stampClass} absolute bottom-2 left-2`}>{recipe.category}</span>
